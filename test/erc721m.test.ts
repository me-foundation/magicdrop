...
describe('ERC721M', function () {
  ...
  describe('Stages', function () {
    ...
    it('reverts when stage end less than start start time', async () => {
      await expect(
        contract.setStages([
          {
            price: ethers.utils.parseEther('0.5'),
            ...
            merkleRoot: ethers.utils.hexZeroPad('0x1', 32),
            maxStageSupply: 5,
            startTimeUnixSeconds: 2,
            endTimeUnixSeconds: 1,
          },
        ]),
      ).to.be.rejectedWith('InvalidStartAndEndTimestamp');
    });
  ...
  describe('Minting', function () {
    ...
    it('enforces stage supply', async () => {
      ...
      ).to.be.rejectedWith('StageSupplyExceeded');
    ...
    it('crossmint with cosign', async () => {
      ...
      const erc721M = await ERC721M.deploy(
        ...
        0,
        owner.address,
        300, // longer expiry for crossmint
      );
      await erc721M.deployed();
