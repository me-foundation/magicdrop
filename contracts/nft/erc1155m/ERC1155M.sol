    ...
    AuthorizedMinterControl
{
    ...
    function setStages(MintStageInfo1155[] calldata newStages) external onlyOwner {
        ...
        for (uint256 i = 0; i < newStages.length; i++) {
            if (i >= 1) {
                if (newStages[i].startTimeUnixSeconds < newStages[i - 1].endTimeUnixSeconds + TIMESTAMP_EXPIRY_SECONDS)
                {
    ...
    function _mintInternal(
        ...
        if (stage.merkleRoot[tokenId] != 0) {
            if (MerkleProof.processProof(proof, keccak256(abi.encodePacked(to, limit))) != stage.merkleRoot[tokenId]) {
                revert InvalidProof();
            }
        ...
        if (_mintCurrency != address(0)) {
            ...
            IERC20(_mintCurrency).safeTransferFrom(
                msg.sender, address(this), (stage.price[tokenId] + adjustedMintFee) * qty
            );
        }
