 ...
 *  - anti-botting
 */
contract Animal is IERC721M, ERC721ACQueryable, Ownable, ReentrancyGuard {
     ...
     * qty - number of tokens to mint
     * to - the address to mint tokens to
     * limit - the limit for the to address
     * proof - the merkle proof generated on client side. This applies if using whitelist.
     * timestamp - the current timestamp
     * signature - the signature from cosigner if using cosigner.
     */
    function crossmintWithLimit(
        uint32 qty,
        address to,
        uint32 limit,
        bytes32[] calldata proof,
        ...
        if (stage.merkleRoot != 0) {
            if (
                MerkleProof.processProof(
                    proof,
                    keccak256(abi.encodePacked(to, limit)) // NOTE: added limit here
                ) != stage.merkleRoot
            ) revert InvalidProof();
