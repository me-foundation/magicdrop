...
export const setTrustedRemote = async (
  ...
  console.log(
    `Setting TrustedRemote on ${hre.network.name}/${args.sourceaddress} to target ${args.targetnetwork}/${args.targetaddress}`,
  );
  ...
  try {
    ...
    await tx.wait();
    console.log(
      `✅ Set TrustedRemote on ${hre.network.name}/${args.sourceaddress} to target ${args.targetnetwork}/${args.targetaddress}`,
    );
  } catch (error) {
    console.log(error);
  }
