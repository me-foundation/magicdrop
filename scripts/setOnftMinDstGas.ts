...
export const setOnftMinDstGas = async (
  ...
  console.log(`Setting min destination gas for ${hre.network.name}/${args.contract} to packetType: ${args.packettype} and minGas: ${args.mingas}`)
  if (!await confirm({ message: 'Continue?' })) return;
  
  try {
    const tx = await contract.setMinDstGas(dstChainId, args.packettype, args.mingas)
