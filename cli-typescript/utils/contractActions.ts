...
import { executeCommand, getCollectionFile, getPasswordIfSet, getTransferValidatorAddress, getTransferValidatorListId, getZksyncFlag, isUnsetOrNull, set1155Uri, setBaseUri, setFundReceiver, setGlobalWalletLimit, setMaxMintableSupply, setMintCurrency, setNumberOf1155Tokens, setRoyalties, setStagesFile, setTokenId, setTokenUriSuffix } from "./common";
import { DEFAULT_MINT_CURRENCY, rpcUrls, TOKEN_STANDARD, TRUE_HEX } from "./constants";
import { confirmSetup, printSignerWithBalance, printTransactionHash } from "./display";
...
export const processStages = async (params: {
    ...
    try {
      ...
      console.error('Error: Failed to get stages data');
    ...
    // Get stagesData from output file and clean up
...
export const setupContract = async (params: {
    ...
    try {
        ...
        checkSetupLocked(contractAddress, rpcUrl, password)
        ...
        if (tokenStandard === TOKEN_STANDARD.ERC1155) {
            ...
            await set1155Uri(title);
            tokenId = await setTokenId(title);
        } else if (tokenStandard === TOKEN_STANDARD.ERC721) {
            baseUri = await setBaseUri(title);
            tokenUriSuffix = await setTokenUriSuffix(title)
        }

        const globalWalletLimit = await setGlobalWalletLimit(tokenStandard, totalTokens, title, tokenId?.toString());
        const maxMintableSupply = await setMaxMintableSupply(tokenStandard, totalTokens, title, tokenId?.toString());
