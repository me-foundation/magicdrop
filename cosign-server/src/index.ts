...
router.post("/collections", async (request: Request & IRequest, event: FetchEvent) => {
	if (!cryptoTimingSafeEqualStr(
...
router.get("/collections", async (request: Request & IRequest, event: FetchEvent) => {
	if (!cryptoTimingSafeEqualStr(
		request.headers.get('x-admin-key') || '',
		ADMIN_KEY,
	)) {
...
router.post("/cosign", async (request: Request & IRequest, event: FetchEvent) => {
	const payload = await request.json<ICosignRequest>()

	// timestamp is unix timestamp in seconds
	const timestamp = Math.floor(new Date().getTime() / 1000);

	const collectionJSONStr = await COLLECTIONS.get(`collection:v1:${payload.collectionContract.toLowerCase()}`);
	if (!collectionJSONStr) {
		return new Response("Collection not found", { status: 404 });
	}
	const collection = JSON.parse(collectionJSONStr) as ICollectionRequest;
	if (collection.startTimeUnixSeconds && timestamp < collection.startTimeUnixSeconds) {
		return new Response("Collection not active", { status: 403 });
	}
	if (collection.endTimeUnixSeconds && timestamp > collection.endTimeUnixSeconds) {
		return new Response("Collection not active", { status: 403 });
	}
	...
	const digest = ethers.utils.solidityKeccak256(
		...
		[payload.collectionContract.toLowerCase(), payload.minter, payload.qty, cosigner.address, timestamp],
	);
	...
	return new Response(JSON.stringify({ sig, timestamp, cosigner: cosigner.address }));
})
